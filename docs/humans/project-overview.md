...
- **双向实时语音**：WebSocket v1 全双工通信，百炼流式 ASR（服务端 VAD）、流式 LLM（`qwen3.7-flash`）与按句百炼流式 TTS（如 `qwen-audio-3.0-tts-flash`）。
- **动态 Agent 配置**：ASR、LLM、TTS、提示词、音色及设备类型映射由数据库管理，并在会话建立时按设备类型装配运行时。
- **扩展与工具调用**：支持内置服务端工具（时间查询、会话关闭），支持 LLM 多轮工具编排。
...
| TTS | 百炼任务型 WebSocket TTS（如 `qwen-audio-3.0-tts-flash` / `longanlingxi`） | 按句流式合成，输出 24 kHz 单声道 PCM |
