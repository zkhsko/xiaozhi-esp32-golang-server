...
> 目标读者：需要快速了解系统能力、关键决策、当前状态与安全限制的人类读者
> 状态：生产代码已包含最小双向语音链路、多数据库支持、设备激活与绑定、Access Token 鉴权及 MCP 工具调用
> 更新日期：2026-08-28
> 详细基线：[架构与需求详细事实基线](../agents/requirements-and-architecture.md)
...
- **网络与安全环境**：公网访问必须由前置反向代理（如 Nginx / Envoy）提供 HTTPS/WSS 保护，敏感凭据（API Key / DSN）必须通过环境变量注入。
- **数据库平滑升级（未决事项）**：复杂生产环境下的数据热迁移和多版本向下兼容策略尚未最终定型。

## 快速指引

- 详细架构规范、API 契约与表结构定义参见：[Agents 详细基线](../agents/requirements-and-architecture.md)。
- 本地启动与快速接入步骤参见：[项目 README](../../README.md)。
