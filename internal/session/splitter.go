...
// 当文本流长时间未出现句末标点时，达到该长度即强制切分输出，防止首音频延迟过高。
const maxSentenceRunes = 30

// SentenceSplitter 实现轻量级增量文本分句器。
// 消费大语言模型输出的流式文本增量，按中英文标点与长度切分为完整句子，流结束时刷新残余文本。
...
func (s *SentenceSplitter) split() []string {
	...
	for len(s.buffer) > 0 {
		...
		for i := 0; i < len(s.buffer); i++ {
			...
			// 只有在已包含实质内容字符后，遇到的标点才作为句末标点切分
			if hasContent && isSentencePunctuation(r) {
				// 排除形如 3.14 的数字中小数点
		...
		// 未达到切分条件
		if splitEnd == -1 {
			if len(s.buffer) >= maxSentenceRunes {
				splitEnd = maxSentenceRunes
			} else {
				break
			}
