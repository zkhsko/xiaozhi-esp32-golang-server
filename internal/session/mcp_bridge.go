...
// Enable 在客户端声明支持 MCP 且握手成功后初始化 DeviceMCPClient 并启动后台发现。
func (b *MCPBridge) Enable(sessionId string, sender DownlinkSender) {
	b.mu.Lock()
	...
	go func() {
		discCtx, cancel := context.WithTimeout(context.Background(), agentkit.DefaultDiscoveryTimeout)
		defer cancel()
...
func (b *MCPBridge) SendMCPPayload(ctx context.Context, payload json.RawMessage) error {
	...
	return sender.SendText(ctx, data)
}
