...
// - user_id: 绑定的用户 ID，非空，联合索引 idx_device_user_ref_user_serial。
// - created_at: 创建时间（绑定时间）。
// - updated_at: 记录最近更新时间。
type DeviceUserRef struct {
	ID           uint64    `gorm:"primaryKey;autoIncrement;column:id" json:"id"`
	SerialNumber string    `gorm:"uniqueIndex:uk_device_user_ref_serial_number;column:serial_number;size:64;not null" json:"serial_number"`
	UserID       uint64    `gorm:"index:idx_device_user_ref_user_serial,priority:1;column:user_id;not null" json:"user_id"`
